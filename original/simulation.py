...
class FSISimulation:
    ...
    def __init__(self, data: dict):
        ...
        self.collision_solver = CollisionSolver(BODY_W / 2.0, BODY_H / 2.0)
    ...
    def _resolve_collisions(self):
        ...
        for i in range(nb):
            bodies.append(dict(
                pos=np.array([self.b_pos[i][0], self.b_pos[i][1]],
                             dtype=np.float64),
                vel=np.array([self.b_vel[i][0], self.b_vel[i][1]],
                             dtype=np.float64),
                ang=float(self.b_ang[i]),
                omg=float(self.b_omg[i]),
                inv_m=BODY_INV_MASS,
                inv_I=BODY_INV_I,
            ))
        self.collision_solver.solve(bodies, dt)
        for i in range(nb):
            self.b_vel[i] = ti.Vector([float(bodies[i]['vel'][0]),
                                       float(bodies[i]['vel'][1])])
            self.b_omg[i] = float(bodies[i]['omg'])

    # ─────────────────────────────────────────────────────
    #  完整子步: 4 kernel + 1 prefix-sum (+ CPU collision)
    # ─────────────────────────────────────────────────────
    ...
    def substep(self):
        ...
        self._resolve_collisions()                  # CPU (skip if nb<2)
    ...
    def update_vis(self):
        ...
        for i in range(na):
            ...
            t = self.ptype[i]
            if t == 0:
                self.c_vis[i] = ti.Vector([0.2, 0.6, 1.0])
            elif t == 1:
